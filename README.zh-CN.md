...
- 可根据名称、描述和域名自动分组。
- 设置面板支持开机启动。
- 设置面板支持窗口行为配置：自动隐藏、边缘唤出、吸附边缘、吸附距离、唤出延迟和全局快捷键。
- 设置面板支持备份与恢复：手动备份、自动备份、导入前恢复点和提示中心。
- 设置面板支持个性化：配置文件、布局预设、主题和面板透明度。
- 搜索栏支持选择搜索引擎，按 Enter 可直接调用搜索引擎搜索。

## v1.2.0

- 新增配置文件切换，适合工作、个人、临时项目等不同场景。
- 新增“复制新建配置”和配置重命名。
- 新增紧凑、标准、宽屏和自定义布局预设。
- 新增更明显生效的主题选择和面板透明度控制。
- 新增搜索栏搜索引擎选择，并可在设置中修改默认搜索引擎。
- 优化软件设置结构，按配置、外观布局、显示搜索、窗口行为、备份和数据整理分组。
- 备份目录、窗口行为、全局快捷键等机器级设置不会随配置文件切换。

## v1.1.6

- 新增设置面板中的“备份与恢复”区域。
- 新增备份目录选择、备份保留数量和立即备份。
- 新增数据变更后的低打扰自动备份。
- 新增导入数据前恢复点，可一键恢复导入前状态。
- 新增提示中心，用于展示导入、导出、备份和恢复失败。

## v1.1.5

- 新增设置面板中的“窗口行为”区域。
- 新增失焦自动隐藏实验开关。
- 新增自动隐藏后的边缘唤出，支持按鼠标所在显示器判断工作区。
- 新增吸附边缘、吸附距离和唤出延迟设置。
- 新增可配置的全局显示/隐藏快捷键，基于 Tauri 官方全局快捷键插件。
- 新增快捷键注册状态提示，失败时会明确提示可能存在冲突。
